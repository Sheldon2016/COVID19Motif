...
typedef int node_id; /**<Type for the id of the nodes in the graph */
const node_id NULL_NODE = INT_MAX;    /**<Null node value */
...
/**
 *
 * This construction function is too slow. It hasn't been used in our motif-problem. [comment from Jiafeng Hu]
...
ARGraph<Node, Edge>::ARGraph(ARGLoader<Node, Edge> *loader) {
	...
	int i, j;
	cout << "n: " << n << endl;
	cout << "Start to add nodes" << endl;
	for (i = 0; i < n; i++) {
		attr.push_back(loader->GetNodeAttr(i));
	}

	cout << "Start to add out edges" << endl;
	for (i = 0; i < n; i++) {
	...
	cout << "Start to add in edges" << endl;
	for (i = 0; i < n; i++) {
...
/**
 * This construction func. is implemented by Jiafeng Hu. The input file is "vf3-format" graph.
* @brief Constructs the graph form a loader.
* @param string input_graph_path
*/
//template<typename Node, typename Edge>
//ARGraph<Node, Edge>::ARGraph(string path) {
//    ifstream ifile(path);
//    ifile >> n;
//    //n = loader->NodeCount();
//    vector<int> in_node_count(n, 0);
//    max_deg_in = max_deg_out = max_degree = 0;
//
//    int i, j, id, label, from, to;
//    cout << "n: " << n << endl;
//    cout << "Start to add nodes" << endl;
//    for (i = 0; i < n; i++) {
//        ifile >> id >> label;
//        assert(id == i);
//        attr.push_back(label);
//    }
//    for (int i = 0; i < n; i++) {
//        NodeVec nv_out;
//        EdgeAttrVector eav_out;
...
//
//        for (j = 0; j < k; j++) {
//            out_attr[i].push_back(Edge());
//            ifile >> from >> to;
//            assert(from == i);
//            out[from].push_back(to);
//            in[i].push_back(to);
//            in_attr[i].push_back(Edge());
//        }
//    }
//
//    cout << "Start to add in edges" << endl;
...
//        int count = in[i].size() + out[i].size();
//        if (count > max_degree) {
//            max_degree = count;
//        }
//    }
...
/**
* This construction func. is implemented by Jiafeng Hu. The input file is "motif-format" graph:
...
* @brief Constructs the graph form a path.
* @param string input_graph_path
*/
template<typename Node, typename Edge>
ARGraph<Node, Edge>::ARGraph(string path) {
	...
	cout<<"directed ";
#endif
	cout << "graph from " << path << " for subgraph matching " << endl;
...
#ifdef UNDIRECTED
		pair<int, int> edge = from > to ? make_pair(to, from) : make_pair(from, to);
		if (edgeset.find(edge) != edgeset.end()) {
			// This edge has existed.
			continue;
		} else edgeset.insert(edge);
		in_attr[from].push_back(Edge());
		out_attr[to].push_back(Edge());
		in[from].push_back(to);
		out[to].push_back(from);
#endif
		out_attr[from].push_back(Edge());
		in_attr[to].push_back(Edge());
		out[from].push_back(to);
...
template<typename Node, typename Edge>
ARGraph<Node, Edge>::ARGraph(const Graph &g) {
	//cout << "Start to load from input graph g."<<endl;
